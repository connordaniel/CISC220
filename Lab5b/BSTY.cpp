...
void BSTY::adjustHeights(NodeT *n) {
	NodeT * curr = n;
	while (curr->data.compare(root->data) != 0) {
		if (curr->parent->left == curr) {
			curr->parent->height = max(curr->height, curr->parent->right->height) + 1;
			curr = curr->parent;
		} else {
			curr->parent->height = max(curr->height, curr->parent->left->height) + 1;
			curr = curr->parent;
		}
	}
}
...
NodeT *BSTY::find(string x) {
	NodeT *curr = root;
	while (curr->left != NULL || curr->data.compare(x) != 0) {
		curr = curr->left;
		if (curr->data.compare(x) == 0) {
			cout << curr->data << endl;
			return curr;
		}
	}
	while (curr->right != NULL || curr->data.compare(x) != 0) {
		curr = curr->right;
		if (curr->data.compare(x) == 0)
			cout << curr->data << endl;
			return curr;
